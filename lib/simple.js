...
const adReply = global.adReply = {
    contextInfo: {
        forwardingScore: 999,
        isForwarded: false,
        externalAdReply: {
            title: "Bot uptime: " + global.u,
            body: "",
            previewType: "PHOTO",
            thumbnail: 'https://telegra.ph/file/43c7cc8462a9a68711937.jpg',
            sourceUrl: 'https://chat.whatsapp.com/DHHxj6cS4m2I97PYRlO0MA',
        }
    }
}
...
exports.makeWASocket = (connectionOptions, options = {}) => {
    ...
    */
    conn.sendMedia = async (jid, path, quoted, options = {}) => {
        ...
        return await conn.sendMessage(jid, { [`${pase}`]: data, mimetype: mime, ...options }, { quoted, ephemeralExpiration: ephemeral })
    }
    ...
    */
    conn.translate = async (text, from, to) => {
        const translate = require('translate-google-api')
            return await translate(text, { from, to: to, })
    }
...
END:VCARD`.trim()
    ...
    */
    ...
    conn.fakeReply = (jid, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid, options) => {
        return conn.sendMessage(jid, { text: text }, { ephemeralExpiration: ephemeral, quoted: { key: { fromMe: fakeJid == conn.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
    }
    conn.reply1 = async (jid, text, quoted, men) => {
        return conn.sendMessage(jid, {
            text: text, jpegThumbnail: await (await fetch(thumbr1)).buffer(), mentions: men
        }, { quoted: quoted, ephemeralExpiration: ephemeral })
    }
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, forwardingScore: 99999, isForwarded: true, ...options })
    }
    conn.send2ButtonDoc = async (jid, content, footerText, button1, id1, button2, id2, quoted, options) => {
        ...
        conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, contextInfo: { mentionedJid: conn.parseMention(content + footerText), forwardingScore: 99999, isForwarded: true }, ...options, ephemeralExpiration: ephemeral } )
    }
    conn.send2ButtonImgDoc = async (jid, buffer, contentText, footerText, button1, id1, button2, id2, quoted, options) => {
        ...
        return await conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, contextInfo: { mentionedJid: conn.parseMention(contentText + footerText) }, ...options })
    }
    ...
    conn.sendButton = async (jid, content, footerText, button1, id1, quoted, options) => {
        ...
        conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, contextInfo: { mentionedJid: conn.parseMention(content + footerText), forwardingScore: 999999, isForwarded: true }, ...options })
    }
    ...
    conn.send3Button = async (jid, content, footerText, button1, id1, button2, id2, button3, id3, quoted, options) => {
        ...
        conn.sendMessage(jid, buttonMessage, { quoted, ephemeralExpiration: ephemeral, contextInfo: { mentions: conn.parseMention(content + footerText), forwardingScore: 99999, isForwarded: true }, ...options })
    }
    ...
    * @param {Object} quoted 
    */
    conn.sendTemplateButtonImg = async (jid, buffer, content, footerText, button1, id1, display, url, quoted, options) => {
        let type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 65536) {
        try { throw { json: JSON.parse(file.toString()) } }
        ...
        const buttons = [
            { index: 1, urlButton: { displayText: display, url: url } },
    ...
    conn.sendTemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, options) => {
        ...
        conn.sendMessage(jid, key, { ephemeralExpiration: ephemeral, mentions: conn.parseMention(content + footerText), contextInfo: { forwardingScore: 99999, isForwarded: true }, ...options })
    }
    conn.send2TemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, btn2, id2, quoted, options) => {
        ...
        conn.sendMessage(jid, key, { quoted, ephemeralExpiration: ephemeral, contextInfo: { mentions: conn.parseMention(content + footerText), forwardingScore: 9999, isForwarded: true } })
    }
    conn.send3TemplateButtonFakeImg = async (jid, buffer, content, footerText, btn1, id1, btn2, id2, btn3, id3, quoted, options) => {
        ...
        conn.sendMessage(jid, key, { quoted, ephemeralExpiration: ephemeral, contextInfo: { forwardingScore: 9999, isForwarded: true, mentions: conn.parseMention(content) } })
    }
